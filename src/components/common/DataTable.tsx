...
export const DataTable = <TData,>({
  ...
  return (
    <div className="w-full bg-white rounded-lg shadow-sm p-4">
      {title && (
        <h3 className="text-lg font-bold text-[#03318C] mb-4">{title}</h3>
      )}
          ...
          <tbody>
            {loading ? (
              ...
              table.getRowModel().rows.map((row, index) => (
                <tr
                  key={row.id}
                  className={index % 2 === 0 ? 'bg-white' : 'bg-[#FAFCFF]'}
                >
        ...
        <div className="mt-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3 py-1 border border-[#B3C9E6] text-[#03318C] text-xs font-bold bg-white rounded disabled:opacity-50"
            >
              Previous
            </button>
